...
export function AdminDashboard({locale,articles,status}:{locale:string;articles:ArticleSummary[];status:SystemStatus|null}){
 ...
 {filtered.length===0?<div className="admin-empty"><strong>Sonuç bulunamadı</strong><span>Arama ifadesini veya durum filtresini değiştirin.</span></div>:<div className="article-table" role="table"><div className="article-row article-row-head" role="row"><span>İçerik</span><span>Durum</span><span>Güncelleme</span><span/></div>{filtered.map(x=><div className="article-row" role="row" key={x.id}><span><Link href={`/${locale}/admin/articles/${x.id}`}><strong>{x.title}</strong></Link><small>{x.locale} · {x.type} · /{x.slug}</small></span><span><span className={`status-badge status-${x.status.toLowerCase()}`}>{labels[x.status]??x.status}</span></span><time dateTime={x.updatedAt}>{new Intl.DateTimeFormat("tr-TR",{dateStyle:"medium"}).format(new Date(x.updatedAt))}</time><Link className="row-action" href={`/${locale}/admin/articles/${x.id}`}>Aç →</Link></div>)}</div>}
 </section></>;
}
