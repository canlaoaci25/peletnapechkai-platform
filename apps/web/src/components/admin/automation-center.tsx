...
export type AutomationScan = { activeLocales:string[]; publishedArticles:number; missingTranslations:number; seoCandidates:number; siteLanguageCandidates:number; reportCandidates:number; runnerEnabled:boolean };
...
const cards = [
  { type:"ContentTranslation", title:"Otomatik içerik çevirisi", description:"Yayındaki içeriklerin eksik dil sürümlerini kalıcı fazlara böler.", count:(scan:AutomationScan)=>scan.missingTranslations },
  { type:"SeoLocalization", title:"Otomatik SEO yerelleştirmesi", description:"Eksik SEO başlıklarını ve açıklamalarını hedef dile göre hazırlar.", count:(scan:AutomationScan)=>scan.seoCandidates },
  { type:"SiteLocalization", title:"Otomatik site dili", description:"Arayüzdeki eksik yerelleştirme anahtarlarını hedef dillere hazırlar.", count:(scan:AutomationScan)=>scan.siteLanguageCandidates },
  { type:"SystemReport", title:"Otomatik sistem raporu", description:"İşleri, hataları ve tamamlanma durumunu ayrıntılı bir raporda toplar.", count:()=>0 },
] as const;
...
export function AutomationCenter({locale,initialScan,initialJobs}:{locale:Locale;initialScan:AutomationScan;initialJobs:AutomationJob[]}) {
  ...
  async function start(type:string,count:number){const report=type==="SystemReport";const result=await Swal.fire({title:report?"Yeni sistem raporu oluşturulsun mu?":"Toplu iş başlatılsın mı?",text:report?"Codex mevcut sistem durumunu inceleyip ayrıntılı bir rapor kaydedecek.":`${count} kayıt kalıcı kuyruğa eklenecek ve fazlar halinde çalışacak.`,icon:"question",showCancelButton:true,confirmButtonText:report?"Rapor oluştur":"Toplu çalıştır",cancelButtonText:"Vazgeç",background:"#151922",color:"#f4f6fa",confirmButtonColor:"#ff7651"});if(!result.isConfirmed)return;setBusy(true);try{await post("/",{type,targetLocales:scan.activeLocales.filter(x=>x!=="tr-TR")})}catch(error){await Swal.fire({title:"İş başlatılamadı",text:error instanceof Error?error.message:"Beklenmeyen bir hata oluştu.",icon:"error",background:"#151922",color:"#f4f6fa"})}finally{setBusy(false)}}
