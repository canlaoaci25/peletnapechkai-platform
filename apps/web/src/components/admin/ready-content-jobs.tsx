...
export function ReadyContentJobs({locale,initialJobs}:{locale:Locale;initialJobs:ReadyContentJob[]}){
  const[jobs,setJobs]=useState(initialJobs),[refreshFailed,setRefreshFailed]=useState(false);
  ...
  useEffect(()=>{const timer=window.setInterval(()=>void refresh(),3000);return()=>window.clearInterval(timer)},[]);
  ...
  return <section className="admin-panel ready-content-jobs" aria-live="polite">
    ...
    {active.length===0?<div className="ready-content-empty"><strong>Devam eden iş yok.</strong><span>Yeni iş başlattığınızda ilerlemesi burada görünecek.</span></div>:<div className="ready-content-job-list">{active.map(job=><JobCard key={job.id} job={job} locale={locale}/>)}</div>}
...
function JobCard({job,locale,compact=false}:{job:ReadyContentJob;locale:Locale;compact?:boolean}){
  ...
  return <article className="ready-content-job" data-status={job.status.toLowerCase()}>
    ...
    <div className="ready-content-job-metrics"><span><b>{job.completedItems}</b> tamamlandı</span><span><b>{remaining}</b> kaldı</span><span><b>{job.failedItems}</b> hata</span></div>
