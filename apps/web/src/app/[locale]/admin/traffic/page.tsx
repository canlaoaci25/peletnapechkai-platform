...
export default async function TrafficPage({ params }: PageProps<"/[locale]/admin/traffic">) {
  ...
  if (!data) return <main className="admin-shell"><p>Trafik verisi alınamadı.</p></main>;

  return <main className="admin-shell admin-dashboard-shell">
    <header className="admin-command-header"><div><p className="section-kicker">BÜYÜME MERKEZİ</p><h1>Trafik ve içerik fırsatları</h1><p>Gerçek BOECL okuma sinyalleri, konu kümeleri ve iyileştirme öncelikleri.</p></div></header>
    ...
    {data.searchConsole && <section className="admin-panel"><p className="section-kicker">GOOGLE SEARCH CONSOLE · SON 28 GÜN</p><h2>Organik arama özeti</h2><div className="overview-kpis"><article><span>Tıklama</span><strong>{data.searchConsole.clicks}</strong></article><article><span>Gösterim</span><strong>{data.searchConsole.impressions}</strong></article><article><span>Ortalama TO</span><strong>%{(data.searchConsole.ctr * 100).toFixed(1)}</strong></article><article><span>Ortalama konum</span><strong>{data.searchConsole.averagePosition.toFixed(1)}</strong></article></div><div className="traffic-table">{data.searchConsole.queries.map((query) => <div key={query.query}><strong>{query.query}</strong><span>{query.clicks} tıklama</span><span>{query.impressions} gösterim</span><span>Konum {query.position.toFixed(1)}</span></div>)}</div></section>}
