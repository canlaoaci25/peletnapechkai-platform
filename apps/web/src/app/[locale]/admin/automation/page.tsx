...
import { AutomationCenter, type AutomationJob, type AutomationScan } from "@/components/admin/automation-center";
...
async function get<T>(path: string) {
  ...
  const response = await fetch(new URL(path, apiUrl), { headers: { cookie: cookieStore.toString() }, cache: "no-store" });
...
export default async function AutomationPage({ params }: PageProps<"/[locale]/admin/automation">) {
  ...
  return <main className="admin-shell admin-dashboard-shell">
    <header className="admin-command-header"><div>
    ...
    <AutomationCenter locale={locale} initialScan={scan} initialJobs={jobs ?? []} />
  </main>;
}
