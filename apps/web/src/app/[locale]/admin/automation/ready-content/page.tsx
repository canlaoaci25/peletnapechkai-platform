...
export default async function ReadyContentPage({params}:PageProps<"/[locale]/admin/automation/ready-content">){
  ...
  const library=await getSupportingLibrary();const categories=library.categories.filter(category=>category.locale==="tr-TR").map(({id,name,slug})=>({id,name,slug}));
  return <main className="admin-shell admin-dashboard-shell ready-content-page"><header className="admin-command-header"><div><p className="section-kicker">AI HAZIR</p><h1>Hazır içerik oluştur</h1><p>Popüler yayınları araştıran, özgünlüğü denetleyen ve seçilen tüm fazlar bittiğinde doğrudan yayımlayan kalıcı içerik işi oluşturun.</p></div></header><ReadyContentForm locale={locale} categories={categories} runnerEnabled={true}/></main>;
}
