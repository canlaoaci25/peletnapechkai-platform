...
export type TrafficDashboard={locale:string;checkedAt:string;published:number;totalViews:number;totalEngagedSeconds:number;averageEngagedSeconds:number;measurement:{internalAnalytics:boolean;ga4:boolean;clarity:boolean;searchConsole:boolean};searchConsole:null|{startDate:string;endDate:string;clicks:number;impressions:number;ctr:number;averagePosition:number;queries:{query:string;clicks:number;impressions:number;ctr:number;position:number}[]};top:{slug:string;title:string;views:number;engagedSeconds:number}[];opportunities:{id:string;slug:string;title:string;views:number;engagedSeconds:number;hasSeo:boolean;hasCover:boolean;tagCount:number;reason:string}[];clusters:{name:string;articles:number;views:number;engagedSeconds:number}[]};
