...
public static class EditorialCollaborationEndpoints
{
    ...
    {var actor=await users.GetUserAsync(principal);var article=await db.ArticleLocalizations.SingleOrDefaultAsync(x=>x.Id==articleId,token);if(actor is null)return Results.Unauthorized();if(article is null)return Results.NotFound();var item=await db.ArticleQualityChecklists.SingleOrDefaultAsync(x=>x.ArticleLocalizationId==articleId,token)??new ArticleQualityChecklist(article);var now=DateTimeOffset.UtcNow;item.Update(request.TitleAndSummary,request.SourcesVerified,request.AuthorAndTaxonomy,request.SeoMetadata,request.CoverAccessibility,request.CommercialDisclosure,request.TranslationReviewed,request.LegalEditorialReview,actor.Id,now);if(db.Entry(item).State==EntityState.Detached)db.Add(item);db.AuditLogs.Add(new AuditLog(actor.Id,"editorial.quality_checklist_updated",nameof(ArticleQualityChecklist),articleId,System.Text.Json.JsonSerializer.Serialize(new{item.IsComplete,missing=PublicationQualityGate.Missing(item)}),now));await db.SaveChangesAsync(token);return Results.Ok(new{item.IsComplete});}
