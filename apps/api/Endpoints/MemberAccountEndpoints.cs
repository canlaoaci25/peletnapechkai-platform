...
public static class MemberAccountEndpoints
{
    ...
    private static async Task<IResult> ListSavedAsync(string? locale,System.Security.Claims.ClaimsPrincipal principal,UserManager<ApplicationUser> users,PublishingDbContext db,CancellationToken token)
    {
        ...
        var items=await query.OrderByDescending(item=>item.SavedAt).Select(item=>new{item.ArticleLocalization.Slug,item.ArticleLocalization.Title,item.ArticleLocalization.Summary,type=item.ArticleLocalization.ArticleGroup.Type.ToString(),locale=item.ArticleLocalization.Locale.Code,item.ArticleLocalization.PublishedAt,item.SavedAt,cover=item.ArticleLocalization.CoverMediaAssetId==null?null:new{url="/api/media/"+item.ArticleLocalization.CoverMediaAssetId+"?v="+item.ArticleLocalization.CoverMediaAsset!.OptimizedByteLength,altText=item.ArticleLocalization.CoverAltText,item.ArticleLocalization.CoverMediaAsset.FocalX,item.ArticleLocalization.CoverMediaAsset.FocalY}}).ToArrayAsync(token);
    ...
    {
        var user=await users.GetUserAsync(principal);if(user is null||!user.IsActive)return Results.Unauthorized();
        var items=await db.ArticleReadingProgress.AsNoTracking().Where(item=>item.UserId==user.Id&&item.Percent>=5&&item.Percent<95&&item.ArticleLocalization.Locale.Code==locale&&item.ArticleLocalization.Status==PublicationStatus.Published).OrderByDescending(item=>item.LastReadAt).Take(8).Select(item=>new{item.ArticleLocalization.Slug,item.ArticleLocalization.Title,item.ArticleLocalization.Summary,locale=item.ArticleLocalization.Locale.Code,item.Percent,item.Anchor,item.LastReadAt,cover=item.ArticleLocalization.CoverMediaAssetId==null?null:new{url="/api/media/"+item.ArticleLocalization.CoverMediaAssetId+"?v="+item.ArticleLocalization.CoverMediaAsset!.OptimizedByteLength,altText=item.ArticleLocalization.CoverAltText,item.ArticleLocalization.CoverMediaAsset.FocalX,item.ArticleLocalization.CoverMediaAsset.FocalY}}).ToArrayAsync(token);
        return Results.Ok(items);
    }
